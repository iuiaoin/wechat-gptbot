...
class WeChatChannel:
    ...
    def handle_single(self, msg):
        ...
        if match_image_prefix:
            context["type"] = const.CREATE_IMAGE
        if match_image_prefix:
            self.send_stable_img(query, sender_id)
        else:
    ...
    def send_stable_img(self,tags,wxid):
        try:
            # download image
            ...
            self.aidraw['text2image']['prompt'] = tags
            self.aidraw['text2image']['seed'] = img_name
            self.aidraw['text2image']['sampler_index'] = 'Euler a'
            self.aidraw['text2image']['steps'] = 20
            self.aidraw['text2image']['cfg_scale'] = 7


            try:
                headers = {'Content-Type': 'application/json', 'Authorization':''}
                response = requests.post('http://127.0.0.1:7860/sdapi/v1/txt2img', data=json.dumps(self.aidraw['text2image']),headers=headers)
